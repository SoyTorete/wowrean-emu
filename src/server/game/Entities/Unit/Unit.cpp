...
bool Unit::WillSpellBeInterrupted(CurrentSpellTypes type, SpellInterruptFlags flag)
{
    if (Spell* spell = GetCurrentSpell(type))
    {
        //Disallow doing anything while preparing to cast spell
        if (spell->getState() == SPELL_STATE_PREPARING)
            return true;
        if (spell->getState() == SPELL_STATE_CASTING)
        {
            if (spell->m_spellInfo->ChannelInterruptFlags & flag)
                return true;
            if (spell->m_spellInfo->InterruptFlags & flag)
                return true;
        }
    }
    return false;
...
bool Unit::CanMoveWhileCasting()
{
    if (HasUnitState(UNIT_STAT_CASTING))
    {
        if (WillSpellBeInterrupted(CURRENT_MELEE_SPELL, SPELL_INTERRUPT_FLAG_MOVEMENT))
            return false;
        if (WillSpellBeInterrupted(CURRENT_GENERIC_SPELL, SPELL_INTERRUPT_FLAG_MOVEMENT))
            return false;
        if (WillSpellBeInterrupted(CURRENT_AUTOREPEAT_SPELL, SPELL_INTERRUPT_FLAG_MOVEMENT))
            return false;
        if (WillSpellBeInterrupted(CURRENT_CHANNELED_SPELL, SPELL_INTERRUPT_FLAG_MOVEMENT))
            return false;
    }
    return true;
}
bool Unit::CanAttackWhileCasting()
{
    if (HasUnitState(UNIT_STAT_CASTING))
    {
        if (WillSpellBeInterrupted(CURRENT_MELEE_SPELL, SPELL_INTERRUPT_FLAG_AUTOATTACK))
            return false;
        if (WillSpellBeInterrupted(CURRENT_GENERIC_SPELL, SPELL_INTERRUPT_FLAG_AUTOATTACK))
            return false;
        if (WillSpellBeInterrupted(CURRENT_AUTOREPEAT_SPELL, SPELL_INTERRUPT_FLAG_AUTOATTACK))
            return false;
        if (WillSpellBeInterrupted(CURRENT_CHANNELED_SPELL, SPELL_INTERRUPT_FLAG_AUTOATTACK))
            return false;
    }
    return true;
}
...
void Unit::SetCurrentCastedSpell(Spell * pSpell)
{
    ...
    switch (CSpellType)
    {
        case CURRENT_GENERIC_SPELL:
        {
            ...
            if (pSpell && pSpell->GetCastTime() > 0)
        ...
        case CURRENT_CHANNELED_SPELL:
        {
            ...
            if (m_currentSpells[CURRENT_AUTOREPEAT_SPELL] &&
                ...
                InterruptSpell(CURRENT_AUTOREPEAT_SPELL);
            if (pSpell && GetSpellDuration(pSpell->GetSpellInfo()) > 0 || GetSpellCastTime(pSpell->GetSpellInfo()))
                AddUnitState(UNIT_STAT_CASTING);
        } break;
...
bool Unit::IsImmunedToSpellEffect(SpellEntry const* spellInfo, uint32 index) const
{
    ...
    if (uint32 aura = spellInfo->EffectApplyAuraName[index])
    {
        ...
        for (AuraEffectList::const_iterator iter = immuneAuraApply.begin(); iter != immuneAuraApply.end(); ++iter)
			if ((spellInfo->Dispel == DISPEL_MAGIC || spellInfo->Dispel == DISPEL_CURSE ||                                // Magic debuff
...
void Unit::setDeathState(DeathState s)
{
    ...
    if (s != ALIVE && s != JUST_ALIVED)
    {
        ...
        //Flying mobs should not continue fly after they die
