...
void Spell::SelectEffectTargets(uint32 i, uint32 cur)
{
    ...
    switch(SpellTargetType[cur])
    {
        ...
        case TARGET_TYPE_DEST_DEST: //5+8+1
        {
            ...
            {
                //Pickup point outside the caster, if possible
                if (dist > m_caster->GetObjectSize())
                    dist = (float)rand_norm()*(dist - m_caster->GetObjectSize()) + m_caster->GetObjectSize();
                else
                    dist = (float)rand_norm()*dist;
            }
