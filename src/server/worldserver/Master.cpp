...
int Master::Run()
{
    ...
    sLog->outString( "%s (WoWReAn Mundo)", _FULLVERSION );
	...
	sLog->outString( "        \\/_/\\/_/\\/_/  \\/_/  \\/_/ World" );
	sLog->outString( "" );
    ...
    #ifdef _WIN32
    {
        ...
        uint32 Aff = sConfig->GetIntDefault("Usando Procesadores:", 0);
        if (Aff > 0)
        {
            ULONG_PTR appAff;
            ULONG_PTR sysAff;
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                sLog->outString("WoWReAn Mundo ha sido habilitada con prioridad ALTA");
            else
